...
function line_segment_intersection(
    ...
    if ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1
        x = l1p1[1] + ua * (l1p2[1] - l1p1[1])
        y = l1p1[2] + ua * (l1p2[2] - l1p1[2])
        return (x, y)
    end
...
function generate_perimeters(contours::Vector{Vector{Vector3D}}, config::PrintConfig)::Vector{Vector{Vector3D}}
    ...
    MIN_SEGMENT_LENGTH = 0.01  # Minimum segment length to consider
    MIN_PERIMETER_LENGTH = config.nozzle_diameter * 3  # Minimum perimeter length
    MIN_ANGLE = 0.01  # Minimum angle between segments (radians)
    MAX_ANGLE = π - 0.01  # Maximum angle between segments
    MIN_AREA = (config.nozzle_diameter * config.nozzle_diameter) * 2  # Minimum area to consider
    ...
    for contour in contours
        ...
        # Ensure proper winding order (counter-clockwise for outer perimeters)
        if area < 0
            reverse!(contour)
        end

        # Clean up the contour
        cleaned_contour = clean_contour(contour, MIN_SEGMENT_LENGTH)
        if length(cleaned_contour) < 3
            continue
        end

        # Add original contour as outer perimeter
        outer_perimeter = ensure_closed_loop(cleaned_contour)
        if valid_perimeter(outer_perimeter, MIN_PERIMETER_LENGTH)
            push!(perimeters, outer_perimeter)
        end

        # Generate inset perimeters
        current_perimeter = outer_perimeter
        for i in 2:config.perimeter_count
            offset_distance = (i - 1) * config.nozzle_diameter
            inset = generate_inset(current_perimeter, offset_distance, MIN_ANGLE, MAX_ANGLE)
            
            if isnothing(inset) || length(inset) < 3
                break
            end

            # Validate the inset
            inset_area = calculate_polygon_area(inset)
            if abs(inset_area) < MIN_AREA
                break
            end

            if valid_perimeter(inset, MIN_PERIMETER_LENGTH)
                push!(perimeters, inset)
                current_perimeter = inset
            else
                break
            end
        end
...
function clean_contour(points::Vector{Vector3D}, min_length::Float64)::Vector{Vector3D}
    cleaned = Vector{Vector3D}()
    n = length(points)
    ...
    for i in 2:n
        current = points[i]
        if vec_distance(last_point, current) >= min_length
...
# Add this with your Vector3D struct definition
Base.copy(v::Vector3D) = Vector3D(v.x, v.y, v.z)

# Also, let's modify ensure_closed_loop to be more explicit
function ensure_closed_loop(points::Vector{Vector3D})::Vector{Vector3D}
    if length(points) == 0
        return points
    end
    
    result = Vector{Vector3D}()
    # Copy all existing points
    for p in points
        push!(result, Vector3D(p.x, p.y, p.z))
    end
    
    # Close the loop if needed
    if vec_distance(result[1], result[end]) > 1e-6
        push!(result, Vector3D(result[1].x, result[1].y, result[1].z))
    end
    
    return result
end

# Modify generate_inset to use explicit construction instead of copy
function generate_inset(perimeter::Vector{Vector3D}, offset::Float64, min_angle::Float64, max_angle::Float64)::Union{Nothing, Vector{Vector3D}}
    inset = Vector{Vector3D}()
    n = length(perimeter)
    ...
    for i in 1:(n-1)
        prev = perimeter[mod1(i-1, n-1)]
        curr = perimeter[i]
        next = perimeter[mod1(i+1, n-1)]
        ...
        angle = acos(clamp(vec_dot(v1, v2), -1.0, 1.0))
        ...
        # Calculate offset scaling based on angle
        offset_scale = 1.0 / max(sin(angle / 2), 0.1)
        
        if !isfinite(offset_scale) || offset_scale > 10.0
            continue
        end
        
        # Generate offset point
        inset_dir = vec_normalize(Vector3D(-bisector.y, bisector.x, 0.0))
        offset_point = vec_add(curr, vec_scale(inset_dir, offset * offset_scale))
        
        if all(isfinite, [offset_point.x, offset_point.y, offset_point.z])
            push!(inset, offset_point)
        end
    end
    
    if length(inset) >= 3 
        # Close the loop with explicit point creation
        if vec_distance(inset[1], inset[end]) > 1e-6
            push!(inset, Vector3D(inset[1].x, inset[1].y, inset[1].z))
        end
        return inset
    else
        return nothing
...
function valid_perimeter(perimeter::Vector{Vector3D}, min_length::Float64)::Bool
    if length(perimeter) < 3
        return false
    end
    
    total_length = 0.0
    for i in 1:(length(perimeter)-1)
        total_length += vec_distance(perimeter[i], perimeter[i+1])
        if !isfinite(total_length)
            return false
        end
    end
    
    return total_length >= min_length && is_perimeter_self_intersecting(perimeter) == false
end

function is_perimeter_self_intersecting(perimeter::Vector{Vector3D})::Bool
    n = length(perimeter)
    for i in 1:(n-2)
        for j in (i+2):(n-1)
            if segments_intersect_2d(perimeter[i], perimeter[i+1], 
...
function segments_intersect_2d(p1::Vector3D, p2::Vector3D, p3::Vector3D, p4::Vector3D)::Bool
    ...
    den = (b[1] - a[1]) * (d[2] - c[2]) - (b[2] - a[2]) * (d[1] - c[1])
    if abs(den) < 1e-10
        return false
    end
    
    ua = ((c[1] - a[1]) * (d[2] - c[2]) - (c[2] - a[2]) * (d[1] - c[1])) / den
    ub = ((b[1] - a[1]) * (c[2] - a[2]) - (b[2] - a[2]) * (c[1] - a[1])) / den
    
    return ua > 0 && ua < 1 && ub > 0 && ub < 1
end

function offset_polygon(polygon::Vector{Vector3D}, distance::Float64)::Vector{Vector3D}
    if length(polygon) < 3 || distance <= 0
    ...
    for i in 1:length(polygon)
        ...
        # Calculate vectors with safety checks
        v1 = vec_sub(curr, prev)
        v2 = vec_sub(next, curr)
        ...
        v1 = vec_scale(v1, 1.0/len1)
        v2 = vec_scale(v2, 1.0/len2)
        
        # Calculate bisector with safety checks
        dot_product = vec_dot(v1, v2)
        if abs(dot_product) > 1.0 - 1e-6  # Nearly parallel vectors
            continue
        end
        
        bisector = vec_normalize(vec_add(v1, v2))
        
        # Calculate offset distance safely
        angle = acos(clamp(dot_product, -1.0, 1.0))
        if angle < 1e-6 || abs(sin(angle/2)) < 1e-6
            continue
        end
        
        offset_dist = distance / sin(angle/2)
        
        # Check for reasonable offset distance
        if !isfinite(offset_dist) || offset_dist > distance * 10
            continue
        end
...
function calculate_polygon_area(polygon::Vector{Vector3D})::Float64
    if length(polygon) < 3
        return 0.0
    end
...
function generate_infill(perimeters::Vector{Vector{Vector3D}}, z::Float64, config::PrintConfig)::Vector{Vector{Vector3D}}
    ...
    # Calculate line spacing based on density
    line_spacing = config.nozzle_diameter * 4.0 / config.infill_density

    if config.infill_pattern == :grid
        return generate_grid_infill(perimeters, z, line_spacing)
    elseif config.infill_pattern == :triangles
        return generate_triangular_infill(perimeters, z, line_spacing)
    elseif config.infill_pattern == :honeycomb
        return generate_honeycomb_infill(perimeters, z, line_spacing)
    else  # Default to grid pattern
        return generate_grid_infill(perimeters, z, line_spacing)
    end
end

function generate_grid_infill(perimeters::Vector{Vector{Vector3D}}, z::Float64, spacing::Float64)::Vector{Vector{Vector3D}}
    infill_paths = Vector{Vector{Vector3D}}()
    ...
    while y <= bounds.max_y
        line = [
            Vector3D(bounds.min_x - spacing, y, z),
            Vector3D(bounds.max_x + spacing, y, z)
        ]
        clipped_lines = clip_line_to_perimeters(line, perimeters)
        append!(infill_paths, clipped_lines)
        y += spacing
    end
    ...
    while x <= bounds.max_x
        line = [
            Vector3D(x, bounds.min_y - spacing, z),
            Vector3D(x, bounds.max_y + spacing, z)
        ]
        clipped_lines = clip_line_to_perimeters(line, perimeters)
        append!(infill_paths, clipped_lines)
        x += spacing
...
function generate_triangular_infill(perimeters::Vector{Vector{Vector3D}}, z::Float64, spacing::Float64)::Vector{Vector{Vector3D}}
    infill_paths = Vector{Vector{Vector3D}}()
    bounds = get_perimeter_bounds(perimeters)
    
    height = spacing * sqrt(3) / 2
    row = 0
    y = bounds.min_y
    ...
    while y <= bounds.max_y
        ...
        while x <= bounds.max_x
            ...
            triangle = [
                Vector3D(x, y, z),
                Vector3D(x + spacing/2, y + height, z),
                Vector3D(x + spacing, y, z),
                Vector3D(x, y, z)  # Close the triangle
            ]
            
            # Clip triangle to perimeters
            clipped_paths = clip_polygon_to_perimeters(triangle, perimeters)
            append!(infill_paths, clipped_paths)
...
function generate_honeycomb_infill(perimeters::Vector{Vector{Vector3D}}, z::Float64, spacing::Float64)::Vector{Vector{Vector3D}}
    infill_paths = Vector{Vector{Vector3D}}()
    bounds = get_perimeter_bounds(perimeters)
    
    # Honeycomb parameters
    hex_width = spacing
    hex_height = hex_width * sqrt(3)
    row_height = hex_height * 3/4
    
    row = 0
    y = bounds.min_y
    
    while y <= bounds.max_y
        x_offset = (row % 2) * (hex_width * 1.5)
        x = bounds.min_x + x_offset
        
        while x <= bounds.max_x
            # Create hexagon points
            hex = [
                Vector3D(x, y + hex_height/2, z),
                Vector3D(x + hex_width/2, y + hex_height, z),
                Vector3D(x + hex_width, y + hex_height/2, z),
                Vector3D(x + hex_width, y - hex_height/2, z),
                Vector3D(x + hex_width/2, y - hex_height, z),
                Vector3D(x, y - hex_height/2, z),
                Vector3D(x, y + hex_height/2, z)  # Close the hexagon
            ]
            
            # Clip hexagon to perimeters
            clipped_paths = clip_polygon_to_perimeters(hex, perimeters)
            append!(infill_paths, clipped_paths)
...
function clip_polygon_to_perimeters(polygon::Vector{Vector3D}, perimeters::Vector{Vector{Vector3D}})::Vector{Vector{Vector3D}}
    # Convert polygon to 2D points for clipping
    polygon_2d = [(p.x, p.y) for p in polygon]
    z = polygon[1].z
    
    clipped_paths = Vector{Vector{Vector3D}}()
    
    for perimeter in perimeters
        perimeter_2d = [(p.x, p.y) for p in perimeter]
        
        # Use Sutherland-Hodgman algorithm for polygon clipping
        clipped_points = sutherland_hodgman_clip(polygon_2d, perimeter_2d)
        
        if length(clipped_points) >= 3
            # Convert back to 3D points
            path = [Vector3D(p[1], p[2], z) for p in clipped_points]
            push!(clipped_paths, path)
        end
    end
...
function sutherland_hodgman_clip(subject::Vector{Tuple{Float64,Float64}}, clip::Vector{Tuple{Float64,Float64}})::Vector{Tuple{Float64,Float64}}
    if length(subject) < 3 || length(clip) < 3
    ...
    for i in 1:length(clip)
        ...
        clip_start = clip[i]
        clip_end = clip[mod1(i+1, length(clip))]
        
        for j in 1:length(input)
            current = input[j]
            prev = input[mod1(j-1, length(input))]
            
            if point_inside_edge(current, clip_start, clip_end)
                if !point_inside_edge(prev, clip_start, clip_end)
                    intersection = line_intersection(prev, current, clip_start, clip_end)
                    if intersection !== nothing
                        push!(output, intersection)
                    end
                end
                push!(output, current)
            elseif point_inside_edge(prev, clip_start, clip_end)
                intersection = line_intersection(prev, current, clip_start, clip_end)
                if intersection !== nothing
                    push!(output, intersection)
                end
...
function point_inside_edge(point::Tuple{Float64,Float64}, edge_start::Tuple{Float64,Float64}, edge_end::Tuple{Float64,Float64})::Bool
    return (edge_end[1] - edge_start[1]) * (point[2] - edge_start[2]) - 
           (edge_end[2] - edge_start[2]) * (point[1] - edge_start[1]) <= 0
end

function line_intersection(p1::Tuple{Float64,Float64}, p2::Tuple{Float64,Float64}, 
                         p3::Tuple{Float64,Float64}, p4::Tuple{Float64,Float64})::Union{Nothing,Tuple{Float64,Float64}}
    # Calculate denominators
